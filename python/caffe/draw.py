    ...
    if layer.type == 'Convolution' or layer.type == 'Deconvolution':
        ...
        node_label = '"%s%s(%s %s)%skernel size: %d%sstride: %d%spad: %d"' %\
                     (layer.name,
                      ...
                      layer.type,
                      separator,
                      layer.pooling_param.kernel_size if layer.pooling_param.HasField("kernel_size") else 1,
                      separator,
                      layer.pooling_param.stride if layer.pooling_param.HasField("stride") else 1,
                      separator,
                      layer.pooling_param.pad if  layer.pooling_param.HasField("pad") else 0)
    else:
